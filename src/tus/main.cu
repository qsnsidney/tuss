...
int main(int argc, char *argv[])
{
    ...
    int shared_mem_size = nthreads * (sizeof(data_t_3d) + sizeof(data_t));
    
    // calculate the initialia acceleration
    calculate_acceleration<<<nblocks, nthreads, shared_mem_size>>>(nBody, d_X[src_index], d_M, d_A[src_index], nthreads);
    ...
    for (unsigned step = 0; step < simulation_time; step += step_size)
    {
        ...
        calculate_acceleration<<<nblocks, nthreads, shared_mem_size>>>(nBody, d_X[src_index], d_M,                                                          //input
                                                        d_A[dest_index], nthreads);                                                                    // output
