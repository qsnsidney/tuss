        ...
        for (int i_iter = 0; i_iter < n_iter; i_iter++)
        {
            ...
            else
            {
                ...
                for (size_t pair_id = 0; pair_id < num_pairs; pair_id++)
                {
                    ...
                    const CORE::ACC tgt_to_src{CORE::universal_field(buf_out.pos.at(j_source_body), buf_out.pos.at(i_target_body))};
                    buf_out.acc[i_target_body] += mass[j_source_body] * tgt_to_src;
                    buf_out.acc[j_source_body] -= mass[i_target_body] * tgt_to_src;
                }
                for (size_t i_target_body = 0; i_target_body < n_body; i_target_body++)
